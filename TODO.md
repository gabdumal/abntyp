# Roadmap

## Introdução

### Legenda

- [⚫] Item não planejado para implementar
- [🔵] Item totalmente concluído
- [🟢] Item cujas atividades planejadas foram finalizadas
- [⚪] Item em implementação

## [⚪] NBR 6022:2018 — Informação e documentação — Artigo em publicação periódica técnica e/ou científica — Apresentação — 2ª edição, 2018-05-16

- [⚪] 5. Estrutura
  - [⚪] 5.1. Elementos pré-textuais
    - [⚪] 5.1.1. Título no idioma do documento (obrigatório)
    - [⚪] 5.1.2. Título em outro idioma (opcional)
    - [⚪] 5.1.3. Autor (obrigatório)
    - [⚪] 5.1.4. Resumo no idioma do documento (obrigatório) — NBR 6028
    - [⚪] 5.1.5. Resumo em outro idioma (opcional) — NBR 6028
    - [⚪] 5.1.6. Datas de submissão e aprovação do artigo (obrigatório)
    - [⚪] 5.1.7. Identificação e disponibilidade (opcional)
  - [⚪] 5.2. Elementos textuais
    - [⚪] 5.2.1. Introdução (obrigatório)
    - [⚪] 5.2.2. Desenvolvimento (obrigatório) — NBR 6024
    - [⚪] 5.2.3. Conclusão (obrigatório)
  - [⚪] 5.3. Elementos pós-textuais
    - [⚪] 5.3.1. Referências (obrigatório) — NBR 6023
      [⚪] 5.3.2. Glossário (opcional)
    - [⚪] 5.3.3. Apêndice (opcional) — NBR 6024
      [⚪] 5.3.4. Anexo (opcional) — NBR 6024
      [⚪] 5.3.5. Agradecimentos (opcional)
- [⚪] 6. Regras gerais
  - [🔵] 6.1. Formato
    - [🔵] Tamanho da fonte
    - [🔵] Espaçamento
  - [🔵] 6.2. Seções — NBR 6024
  - [⚪] 6.3. Citações e notas — NBR 10520
  - [🔵] 6.4. Sigla
  - [⚪] 6.5. Equações e fórmulas
  - [⚪] 6.6. Ilustrações
    - [⚪] Numeração
    - [⚪] Legenda
      - [⚪] Legenda
      - [⚪] Fonte — NBR 10520
      - [⚪] Notas
  - [⚪] 6.7. Tabelas — IBGE

## [🟢] NBR 6023:2025 — Informação e documentação — Referências — Elaboração — 3ª edição, 2025-05-21

- [🔵] 4. Elementos da referência
- [🔵] 5. Localização
- [🔵] 6. Regras gerais de apresentação
- [🔵] 7. Modelos de referências
- [🔵] 8. Transcrição dos elementos
- [🟢] 9. Ordenação das referências
  - [🔵] 9.1. Sistema alfabético
  - [⚫] 9.2. Sistema numérico

## [🔵] NBR 6024 — Informação e documentação — Numeração progressiva das seções de um documento — Apresentação — 2ª edição, 2012-02-01

- [🔵] 4. Regras gerais
  - [🔵] 4.1. Seções
    - [🔵] Algarismos
    - [🔵] Limite
    - [🔵] Alinhamento
    - [🔵] Títulos não numerados
    - [🔵] Títulos com mais de uma linha
    - [🔵] Destaques da hierarquia
  - [🔵] 4.2. Alínea
    - [🔵] Pontuação
    - [🔵] Numeração
    - [🔵] Alinhamento
  - [🔵] 4.3. Subalínea
    - [🔵] Pontuação
    - [🔵] Alinhamento
  - [🔵] 4.4 Indicativos

## [⚪] NBR 6027:2012 — Informação e documentação — Sumário — Apresentação — 2ª edição, 2012-12-11

- [⚪] 4. Localização
  - [🟢] 4.1. Em monografias
    - [🔵] Verso e anverso
    - [⚫] Considerar toda a obra em volumes
  - [⚪] 4.2. Em publicações periódicas
- [🟢] 5. Estrutura
  - [🔵] Numeração — NBR 6024
  - [🔵] Alinhamento
  - [🔵] Grafia
  - [🔵] Paginação
  - [⚫] Outros idiomas
- [🔵] 6. Regras gerais

## [⚪] NBR 6028:2021 — Informação e documentação — Resumo, resenha e recensão — Apresentação — 2ª edição, 2021-05-18

- [🔵] 3. Regras gerais
- [⚪] 4. Resumo, resenha e recensão
  - [⚪] 4.1. Resumo
    - [🔵] Estrutura
    - [🔵] Referências
    - [🔵] Palavras-chave
    - [⚪] Como documento autônomo
  - [⚪] 4.2. Resenha e recensão

## [⚫] NBR 6029:2023 — Informação e documentação — Livros e folhetos — Apresentação — 3ª edição, 2023-03-22

## [⚫] NBR 6034:2004 — Informação e documentação — Índice — Apresentação — 2ª edição, 2004-12-31

- [⚫] 4. Classificação
- [⚫] 5. Localização
- [⚫] 6. Regras gerais de apresentação
  - [⚫] Conteúdo
  - [⚫] Organização
  - [⚫] Título
  - [⚫] Apresentação
  - [⚫] Remissiva
  - [⚫] Indicativo

## [⚪] NBR 10520:2023 — Informação e documentação — Citações em documentos — Apresentação — 2ª edição, 2023-07-19

- [🔵] 5. Regras gerais
- [⚪] 6. Sistema de chamada
  - [⚪] 6.1. Sistema autor-data
    - [⚪] 6.1.1. Indicação de responsabilidade
      - [⚪] 6.1.1.1. Pessoa física
      - [⚪] 6.1.1.2. Pessoa jurídica
      - [⚪] 6.1.1.3. Instituição governamental
      - [⚪] 6.1.1.4. Sem autoria
    - [⚪] 6.1.2. _et. al._
    - [⚪] 6.1.3. Data e página parentética
    - [⚪] 6.1.4. Data e página na sentença
    - [⚪] 6.1.5. Citações de diferentes autores com nome e data iguais
    - [⚪] 6.1.6. Citações do mesmo autor com data iguais
    - [⚪] 6.1.7. Mais de uma citação do mesmo autor colocadas em sequência
    - [⚪] 6.1.8. Mais de uma citação de autores diferentes colocadas em sequência
  - [⚫] 6.2. Sistema numérico
  - [⚪] 7. Citação
    - [⚪] 7.1. Citação direta
      - [⚪] 7.1.1. Citação direta longa
      - [⚪] 7.1.3. a 7.1.6. Notas extra
    - [🔵] 7.2. Citação indireta
    - [⚪] 7.3. Citação de citação
  - [p] 8. Notas
    - [⚫] 8.1. Notas de referência
    - [🔵] 8.2. Notas explicativas

## [⚪] NBR 10719:2025 — Informação e documentação — Relatório técnico e/ou científico — Apresentação — 5ª edição, 2025-12-05

- [⚪] 4. Estrutura
  - [⚪] 4.1. Parte externa
    - [⚪] 4.1.1. Capa (opcional)
    - [⚫] 4.1.2. Lombada (opcional) — NBR 12225
  - [⚪] 4.2. Parte interna
    - [⚪] 4.2.1. Elementos pré-textuais
      - [⚪] 4.2.1.1. Folha de rosto (obrigatório)
      - [⚪] 4.2.1.2. Errata (opcional)
      - [⚪] 4.2.1.3. Agradecimentos (opcional)
      - [⚪] 4.2.1.4. Resumo (obrigatório) — NBR 6028
      - [⚪] 4.2.1.5. Lista de ilustrações (opcional)
      - [⚪] 4.2.1.6. Lista de tabelas (opcional)
      - [⚪] 4.2.1.7. Lista de abreviaturas e siglas (opcional)
      - [⚪] 4.2.1.8. Lista de símbolos (opcional)
      - [⚪] 4.2.1.9. Sumário (obrigatório) — NBR 6027
    - [⚪] 4.2.2. Elementos textuais
    - [⚪] 4.2.3. Elementos pós-textuais
      - [⚪] 4.2.3.1. Referências (obrigatório para quando houver citações) — NBR 6023
      - [⚪] 4.2.3.2. Glossário (opcional)
      - [⚪] 4.2.3.3. Apêndice (opcional)
      - [⚪] 4.2.3.4. Anexo (opcional)
      - [⚫] 4.2.3.5. Índice (opcional) — NBR 6034
      - [⚪] 4.2.3.6. Formulário de identificação (obrigatório quando não forem utilizados os dados internacionais de catalogação na publicação)
- [⚪] 5. Regras gerais
  - [⚪] 5.1. Formato
  - [⚪] 5.2. Paginação
    - [⚪] Contagem
    - [⚪] Numeração
    - [⚫] Volumes
    - [⚪] Apêndices e anexos
  - [⚪] 5.3. Numeração progressiva — NBR 6024
  - [⚪] 5.4. Títulos sem indicativo numérico
  - [⚪] 5.5. Citações e notas de rodapé — NBR 10520
  - [⚪] 5.6. Siglas
  - [⚪] 5.7. Equações e fórmulas
  - [⚪] 5.8. Ilustrações
    - [⚪] Numeração
    - [⚪] Legenda
      - [⚪] Título
      - [⚪] Fonte — NBR 10520
      - [⚪] Notas
  - [⚪] 5.9. Tabelas
    - [⚪] Numeração
    - [⚪] Legenda
      - [⚪] Título
      - [⚪] Fonte — NBR 10520
      - [⚪] Notas
    - [⚪] Apresentação — IBGE

## [🟢] NBR 14724:2024 — Informação e documentação — Trabalhos acadêmicos — Apresentação — 4ª edição, 2025-12-16, corrigida em 2025-04-01

- [🟢] 4. Estrutura
  - [🟢] 4.1. Parte externa
    - [🔵] 4.1.1. Capa (obrigatório)
    - [⚫] 4.1.2. Lombada (opcional) — NBR 12225
  - [🟢] 4.2. Parte interna
    - [🔵] 4.2.1. Elementos pré-textuais
      - [🔵] 4.2.1.1. Folha de rosto (obrigatório)
      - [🔵] 4.2.1.2. Errata (opcional)
      - [🔵] 4.2.1.3. Folha de aprovação (obrigatório)
      - [🔵] 4.2.1.4. Dedicatória (opcional)
      - [🔵] 4.2.1.5. Agradecimentos (opcional)
      - [🔵] 4.2.1.6. Epígrafe (opcional)
        - [🔵] Em início de capítulo
      - [🔵] 4.2.1.7. Resumo na língua vernácula (obrigatório) — NBR 6028
      - [🔵] 4.2.1.8. Resumo em língua estrangeira (obrigatório) — NBR 6028
      - [🔵] 4.2.1.9. Lista de ilustrações (opcional)
        - [🔵] Separar numeração da legenda com travessão
      - [🔵] 4.2.1.10. Lista de tabelas (opcional)
        - [🔵] Separar numeração da legenda com travessão
      - [🔵] 4.2.1.11. Lista de abreviaturas e siglas (opcional)
      - [🔵] 4.2.1.12. Lista de símbolos (opcional)
      - [🔵] 4.2.1.13. Sumário (obrigatório) — NBR 6027
    - [🔵] 4.2.2. Elementos textuais
    - [🟢] 4.2.3. Elementos pós-textuais
      - [🔵] 4.2.3.1. Referências (obrigatório) — NBR 6023
      - [🔵] 4.2.3.2. Glossário (opcional)
      - [🔵] 4.2.3.3. Apêndice (opcional)
      - [🔵] 4.2.3.4. Anexo (opcional)
      - [⚫] 4.2.3.5. Índice (opcional) — NBR 6034
- [🟢] 5. Regras gerais
  - [🔵] 5.1. Formato
    - [🔵] Formato do papel
    - [🔵] Margens
    - [🔵] Verso e anverso
    - [🔵] Tamanho da fonte
  - [🔵] 5.2. Espaçamento
    - [🔵] Entre linhas
      - [🔵] Entre texto comum
      - [🔵] Entre texto menor
      - [🔵] Entre referências
    - [🔵] Natureza
    - [🔵] 5.2.1. Notas de rodapé
    - [🔵] 5.2.2. Indicativos de seção
      - [🔵] Espaçamento
      - [🔵] Verso e anverso
    - [🔵] 5.2.3. Títulos sem indicativo numérico
    - [🔵] 5.2.4. Elementos sem título e sem indicativo numérico
  - [🟢] 5.3. Paginação
    - [🔵] Contagem
    - [🔵] Numeração
    - [⚫] Volumes
    - [🔵] Apêndices e anexos
  - [🔵] 5.4. Numeração progressiva — NBR 6024
  - [🔵] 5.5. Citações — NBR 10520
  - [🔵] 5.6. Siglas
  - [🔵] 5.7. Equações e fórmulas
  - [🔵] 5.8. Ilustrações
    - [🔵] Numeração
    - [🔵] Legenda
      - [🔵] Título
      - [🔵] Fonte — NBR 10520
      - [🔵] Notas
  - [🔵] 5.9. Tabelas
    - [🔵] Numeração
    - [🔵] Legenda
      - [🔵] Título
      - [🔵] Fonte — NBR 10520
      - [🔵] Notas
    - [🔵] Apresentação — IBGE

## [⚪] NBR 15287:2025 — Informação e documentação — Projeto de pesquisa — Apresentação — 3ª edição, 2025-03-18

- [⚪] 4. Estrutura
  - [⚪] 4.1. Parte externa
    - [⚪] 4.1.1. Capa (obrigatório)
    - [⚫] 4.1.2. Lombada (opcional) — NBR 12225
  - [⚪] 4.2. Parte interna
    - [⚪] 4.2.1. Elementos pré-textuais
      - [⚪] 4.2.1.1. Folha de rosto (obrigatório)
      - [⚪] 4.2.1.2. Lista de ilustrações (opcional)
      - [⚪] 4.2.1.3. Lista de tabelas (opcional)
      - [⚪] 4.2.1.4. Lista de abreviaturas e siglas (opcional)
      - [⚪] 4.2.1.5. Lista de símbolos (opcional)
      - [⚪] 4.2.1.6. Sumário (obrigatório) — NBR 6027
    - [⚪] 4.2.2. Elementos textuais
    - [⚪] 4.2.3. Elementos pós-textuais
      - [⚪] 4.2.3.1. Referências (obrigatório) — NBR 6023
      - [⚪] 4.2.3.2. Glossário (opcional)
      - [⚪] 4.2.3.3. Apêndice (opcional)
      - [⚪] 4.2.3.4. Anexo (opcional)
      - [⚫] 4.2.3.5. Índice (opcional) — NBR 6034
- [⚪] 5. Regras gerais
